...
name: review-tests
description: |
  Enforce black-box, behavior-driven test standards. Use when reviewing
  unit tests, writing new tests, refactoring tests, or running /review-tests.
...
## QuantumBank — Flask + pytest (this repo)

Ported from Tone Dragons **review-tests**; this section is **project-specific**.

- **HTTP black box:** Use the shared `client` fixture in `test/conftest.py` (`app.test_client()`, lazy `from app import app`). Assert status codes, `Location` on redirects, JSON bodies, and **small stable substrings** — not full HTML snapshots.
- **Discovery:** `pytest.ini` sets `testpaths = test`, `pythonpath = .`, and markers **`public`**, **`banking`**, **`api`**. Test modules must be named `test_*.py`.
- **Markers:** `public` (no session), `banking` (login + HTML banking routes), `api` (JSON under `/api/`). Put new tests in `test_public_routes.py`, `test_banking_routes.py`, or `test_api_routes.py` unless a new surface clearly needs its own file.
...
**Python red flags (same spirit as the table above):**
